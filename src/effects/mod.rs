...
/// Full-view fog, present only on the `forest` level.
#[derive(Component, Default)]
...
fn advance_fog(
    time: Res<Time<Real>>,
    mut materials: ResMut<Assets<FogMaterial>>,
    mut query: Query<(&mut FogEffect, &MeshMaterial2d<FogMaterial>)>,
) {
    for (mut effect, handle) in &mut query {
        effect.elapsed += time.delta_secs();
        if let Some(mut material) = materials.get_mut(&handle.0) {
            material.params.time = effect.elapsed;
        }
            ...
            Some(flight) => {
                ...
                if t >= 1.0 {
                    if let Some(entity) = firefly.particle.take() {
                        // The dot is a separate entity; a level unload may have
                        // taken it already.
                        commands.entity(entity).try_despawn();
                    }
                    firefly.timer = 0.0;
...
pub fn spawn_ambient_effects(commands: &mut Commands, level_name: &str, level_size: Vec2) {
    ...
    match level_name {
        "forest" => {
            ...
            commands.spawn((FogEffect::default(), LevelEntity, Name::new("FogEffect")));
        }
        "forest-1" => {
            for _ in 0..48 {
